...
fn find_trails(
    ...
    if graph[s_row as usize][s_col as usize] == 9 {
        trails.insert(format!("{:?}", path));
        return;
    }
    for dir in directions {
        let mut visited_copy = visited.clone();
        let mut path_copy = path.clone();
        find_trails(graph, directions, trails, &mut visited_copy, &mut path_copy, s_row+dir.0, s_col+dir.1, graph[s_row as usize][s_col as usize], start_r, start_e);
    }
}
